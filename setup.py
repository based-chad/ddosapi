...
if sudo == 'y':
    ...
    subprocess.run("sudo git clone https://github.com/based-chad/ddosapi.git",shell=True)
    subprocess.run("cd ddosapi",shell=True)
    ...
    subprocess.run("sudo apt install git",shell=True)
    subprocess.run("git clone https://github.com/based-chad/ddosapi.git",shell=True)
    subprocess.run("cd ddosapi",shell=True)
    subprocess.run("pip3 install flask",shell=True)
    os.system("clear")
    print("\033[1;31m Setup Done! You are a complete Skid Now.")
